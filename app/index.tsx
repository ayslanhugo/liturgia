...
const IndexScreen = () => {
  ...
  const dynamicFontSize = screenWidth < 375 ? 18 : 22; // Ajuste para telas menores
  ...
  useEffect(() => {
    const loadReadings = async () => {
      try {
        ...
        setError("Erro ao carregar as leituras. Tente novamente.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const renderScene = SceneMap({
    primeira: () => readings && (
      <TabContent
        title="1ª Leitura"
        subtitle="Primeira Leitura"
        content={`${readings.primeiraLeitura.titulo}\n${readings.primeiraLeitura.texto}`}
      />
    ),
    salmo: () => readings && (
      <TabContent
        title="Salmo"
        subtitle="Responsório"
        content={`${readings.salmo.referencia}\n${readings.salmo.refrão}\n${readings.salmo.texto}`}
      />
    ),
    segunda: () => {
    ...
    evangelho: () => readings && (
      <TabContent
        title="Evangelho"
        subtitle="Evangelho do Dia"
        content={`${readings.evangelho.titulo}\n${readings.evangelho.texto}`}
      />
    ),
  });
  ...
  return (
    <View style={styles.container}>
      {loading ? (
        ...
        <Text style={styles.errorMessage}>{error || 'Não foi possível carregar as leituras. Tente novamente mais tarde.'}</Text>
      )}
    </View>
  );
};
